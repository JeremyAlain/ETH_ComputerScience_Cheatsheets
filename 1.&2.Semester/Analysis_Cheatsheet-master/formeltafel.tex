...
\pagebreak
(Leibnizregel) $(fg)^n = \sum_{k = 0}{n}$${n}\choose {k}$$\cdot f^{k}g^{n-k}$

\subsection[Trigon. \& Additionstheoreme]{Trigonom. Funktionen \& Additionstheoreme}
\begin{itemize}[leftmargin=*]
	\item $\sin^2(x) + \cos^2(x) = 1$ $\sin^2(x)=\frac{1}{2} - \frac{1}{2}\cdot (\cos(2x))$
	
	% sinus equations
	\item $\sin(90^\circ \pm \alpha) = \cos(\alpha)$ 
	\item $\sin(180^\circ \pm \alpha) = \mp \sin(\alpha)$
	\item $\sin(\alpha \pm \beta) = \sin(\alpha)\cos(\beta) \pm	\cos(\alpha)\sin(\beta)$
	\item $\sin(2\alpha) = 2 \sin(\alpha)\cos(\alpha)\ oder \cos(\alpha)\sin(2\alpha) =\frac{1}{2}\sin(\alpha)$
...
\begin{itemize}[leftmargin=*]
	...
	\item $n^2 = \sum_{k=1}^{n}(2k-1) = n^2$
	\item $\sum_{2}^{\infty} f(x)< \infty$ ist monoton fallend gerade dann wenn gilt: $ f>0, f'<0, \int_{2}^{\infty} existiert$
\end{itemize}
...
\begin{itemize}[leftmargin=*]
	...
	\item $\lim_{n \to \infty} \ln(n) \rightarrow \infty$
...
\begin{itemize}[leftmargin=*]
	...
	\item 
	\begin{minipage}{0.35\columnwidth}
		$\sum_{n=0}^\infty a_{0}q^n = \frac{a\small{0}}{1-q}$
	\end{minipage}
	\begin{minipage}{0.60\columnwidth}
...
\subsection{Komplexe Zahlen}
	...
	$\sqrt{-1} = i \rightarrow -\frac{1}{i} = i ; i$ ist imaginäre Einheit
	\\$z = a + bi; z - a = bi $(Spiegelung and reeller Achse); wobei$ a = Re(z)$ und $b=Im(z); r = \sqrt{(x^2 + y^2)}$ \\
	\subsubsection{Rechenregeln}

	für $ z = a + bi$ und $x = c + di $ \\
		\begin{itemize}
	\item $z \pm x = (a \pm c) + (b\pm d)i$ \\
	\item $z\cdot\overline{z} = (x+iy)(x-iy)$\\
	\item $|z|^2 = z \cdot \bar{z} = (a + b\cdot i) \cdot (a - b\cdot i) = a^2 + b^2$ \\
	\item $ z\cdot x = (ac - bd) +i(bc+ad) = x^2 - i^2y^2 = x^2 + y^2$ \\
	\item $r = |z| = \sqrt{a^2 + b^2} = \sqrt{z\cdot \overline{z}}$\\
	\item $\frac{1}{x} = \frac{x}{|X|^2} \rightarrow \frac{z}{x} = \frac{zx}{|x|^2}$ \\
	\item $ z\cdot n = n\cdot a + i\cdot n\cdot b$ \\
	\item $arg(z) = \phi; arg(z) = -arg(z)$ \\
	\item $arg(\frac{z1}{z2}) = arg(z1) - arg(z2)$ \\
	\item$ Re(z) = \frac{z +\overline{z} }{2} ; Im(z) = \frac{z-\overline{z}}{2i}$ \\
	\item $ \overline{(z\pm x)} = \overline{z} \cdot \overline{x}; Re(z) \leq \overline{z}; IM(z) \leq \overline{z}$ \\
	\item $|z+x| \leq |z| + |x|$ \\
	\item $||zw||^2 = (zw)(\overline{zw}) = zwz\overline{w} = ||z||^2 + ||w||^2$ 
	...
	\begin{itemize}
	 ...
	 \item $z = a + bi = re^{i\phi} = r(\cos\phi+i\sin\phi)$\\
	 \item $\overline{z} = r\cdot e^{-i\phi} = r(cos(\phi) - isin(\phi))$\\
	 \item $ \frac{z}{\overline{z}} = \frac{r_z}{r_{\overline{z}}} \cdot e^{\phi_z - \phi_{\overline{z}}}$ = 1\\
	 \item $me^{i\phi}\cdot ne^{i\phi} = m\cdot n \cdot e^{i(\phi + \theta)}$\\
	 \item $(re^{\phi i})^{n} = r^ne^{ni\phi}$\\
	 \item $e^{-2i} = e^{-i\cdot (2)} = \cos(2) - i\sin(2)$\\
	\end{itemize}	
	
	\textbf{Additionstheoreme} \\
	\begin{itemize}
			
	\item $ e^{r\phi}e^{i\theta}$
	\item $(c,d) \rightarrow (ac - bd, ad + bc) \epsilon R^2 \rightarrow (\cos\phi + i\sin\theta)(\cos\phi +i\sin\theta)$ $=(\cos\phi\cos\theta - \sin\phi\sin\theta)+  i(\sin\phi\cos\theta + \cos\phi\sin\theta)\\= \cos(\phi+\theta)+i\sin(\phi+\theta) = e^{i(\phi+\theta)}$
	\end{itemize}
...
%		\end{enumerate}
%		\item Falls Nein:\\
%		Wir sind fertig. Wenn noch ein Rest übrig bleibt, muss dies dem Ergebnis noch hinzugefügt werden. Also Rest / Nenner noch dem Ergebnis addieren werden. 
%	\end{enumerate}
%\end{enumerate}
%\textbf{Beispiel} \\
